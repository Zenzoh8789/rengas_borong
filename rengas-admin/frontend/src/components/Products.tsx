...
export function Products({
  ...
  async function remove(id: number) {
    ...
    try {
      ...
      setToast({ type: "error", message: "Product could not be deleted" });
    }
  }
  ...
  return (
    <section className="panel">
      ...
      <div className="product-table">
        ...
        {loading ? (
            ...
            <div className="product-row" key={product.id}>
              ...
              <img
                className={`product-image ${
                  product.imageUrl ? "" : "default-product-logo"
                }`}
              ...
              <b>RM {Number(product.price).toFixed(2)}</b>
              <span className="actions">
                <button onClick={() => setViewing(product)}>
                  <Eye />
                  View
                </button>
                <button title="Edit" onClick={() => setEditing(product)}>
                  <Pencil />
                  Edit
                </button>
                <button className="delete" onClick={() => remove(product.id)}>
                  <Trash2 />
                </button>
              </span>
            </div>
          ...
          <div className="empty">
            ...
            <p>Search/filter does not match any products.</p>
          </div>
        )}
      </div>
      ...
      {viewing && (
        <ProductView product={viewing} close={() => setViewing(null)} />
      )}
      {editing && (
        <EditProduct
          product={editing}
